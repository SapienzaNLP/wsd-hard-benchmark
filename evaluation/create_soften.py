    ...
    for path in paths:
        with open(path) as f:
            for line in f:
                ...
                if instance_id not in intersection_instance_ids:
...
def create_softEN(all_data, all_keys, data_output_path, keys_output_path) -> None:
    root = ET.Element('corpus')
    root.attrib['language'] = 'en'
    root.attrib['source'] = 'softEN'
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to softEN data (xml).'
    )
    parser.add_argument(
        '--gold_output_path',
        type=str,
        required=True,
        help='Path to softEN keys (txt).'
    )
